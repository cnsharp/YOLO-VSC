...
import { resolveAgents, type AgentDef } from "../agents";
import { canExecute, resolvePath } from "../agentDetector";
...
export class YoloViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(private readonly extensionUri: vscode.Uri) {
    ...
    vscode.workspace.onDidChangeConfiguration((e) => {
      // All of this product's settings live under `yolo.*` — including the agent catalog (`yolo.agents`).
  ...
  public reveal(): void {
    // The view lives in the Activity Bar (primary side bar). Focusing it reveals the container and the
    // docked webview view, creating it on first use.
