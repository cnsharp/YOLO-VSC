...
内置目录随仓库根目录的 `agents.json` 发布（图标位于 `media/agents/`）。下拉框只显示显示名，不显示 command / id。

| id | 显示名 | command (PATH) | YOLO (skipFlag) | Resume (resumeFlag) |
|---|---|---|---|---|
...
| goose | Goose | `goose` | *(环境变量 `GOOSE_MODE=auto`)* | `-r` |
