...
WARNING: This link could potentially be dangerous`)) {
  ...
  var PRIORITY = { member: 4, type: 3, file: 2, url: 1 };
  var matchers = [];
  var agents = [];
  var selectedAgentId = "";
  ...
  function renderSkipToggle() {
    const btn = document.getElementById("skipToggle");
    if (!btn || !skipIcons) {
      return;
    }
  ...
  function renderAgentMenu() {
    ...
    menu.innerHTML = visible.map((a) => {
      const logo = a.iconUri ? `<img class="logo" src="${escapeAttr(a.iconUri)}" alt="" />` : `<span class="logo lightning" title="no icon set">\u26A1</span>`;
    ...
    menu.querySelectorAll(".agent-opt").forEach((el) => {
  ...
  window.addEventListener("message", (ev) => {
    ...
    switch (msg.type) {
      case "init":
        ...
        renderAgentMenu();
        if (!selectedAgentId) {
          const remembered = msg.lastAgentId ? agents.find((a) => a.id === msg.lastAgentId && a.resolvedPath) : void 0;
  ...
  function buildTerminalTheme() {
    const bg = readVscodeColor("--vscode-sideBar-background") ?? readVscodeColor("--vscode-editor-background") ?? "#1e1e1e";
    const fg = readVscodeColor("--vscode-terminal-foreground") ?? readVscodeColor("--vscode-editor-foreground") ?? "#cccccc";
    const cursor = readVscodeColor("--vscode-terminalCursor-foreground") ?? readVscodeColor("--vscode-editorCursor-foreground") ?? "#1E64B4";
    const selBg = readVscodeColor("--vscode-terminal-selectionBackground") ?? readVscodeColor("--vscode-editor-selectionBackground");
    const theme = { background: bg, foreground: fg, cursor };
    if (selBg) {
  ...
  function applyTheme() {
    ...
    if (wrap) {
      wrap.style.background = readVscodeColor("--vscode-sideBar-background") ?? readVscodeColor("--vscode-editor-background") ?? "";
    }
  }
  var term;
  function num(s) {
  ...
  function buildPayload(m, match) {
    switch (m.kind) {
      case "url":
        return { kind: "url", url: match[0] };
      case "file": {
        const path = m.fields ? match[m.fields.path] : void 0;
        if (!path) {
          return void 0;
        }
      ...
      case "type": {
        ...
        return name ? { kind: "type", name } : void 0;
      }
      case "member": {
        const className = match.groups?.class;
        const member = match.groups?.member;
        return className && member ? { kind: "member", className, member } : void 0;
      }
    }
    return void 0;
  }
  function provideLinks(lineText) {
    const candidates = [];
  ...
  function initTerminal() {
    ...
    const t = new import_xterm.Terminal({
    ...
    function cellXForCharIndex(line, charIndex) {
      let chars = 0;
      let x = 0;
    ...
    t.registerLinkProvider({
      provideLinks: (bufferLineNumber, callback) => {
        ...
        const links = provideLinks(text).map((c) => ({
          range: {
